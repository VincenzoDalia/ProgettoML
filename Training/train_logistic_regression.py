...
def comparation_plot(D, L, prior):
    ...
    plt.xscale("log")
    plt.ylabel("minDCF")
    plt.title("Logistic Regression Comparation Graph")
    
    plt.plot(l_values, min_dcf_results_raw, label="minDCF(RAW)", color='#04388c')
    plt.plot(l_values, min_dcf_results_znorm, label="minDCF(Z-norm)",  color='#3a76d6')
    plt.plot(l_values, min_dcf_results_pca_11, label="minDCF(PCA 11)",  color='#75120b')
    plt.plot(l_values, min_dcf_results_pca_znorm_11, label="minDCF(PCA 11 + Z-norm)",  color='#e6473c')
    
    plt.xlim(l_values[0], l_values[-1])
    plt.legend(loc='upper left')
    plt.savefig("Training/Logistic_Regression_Plot/Logistic Regression Comparation Graph.pdf")
    plt.close()
...
def comparation_plot_2(D, L, prior):
    ...
    plt.plot(l_values, min_dcf_results_pca_10, label="minDCF(PCA 10)",  color='#0c700a')
    plt.plot(l_values, min_dcf_results_pca_znorm_10, label="minDCF(PCA 10 + Z-norm)",  color='#98fa96')
    plt.plot(l_values, min_dcf_results_pca_9, label="minDCF(PCA 9)", color='#940aa1')
    plt.plot(l_values, min_dcf_results_pca_znorm_9, label="minDCF(PCA 9 + Z-norm)",  color='#f29bfa')
...
def different_Pi_T_Raw_Graph(D, L):
    ...
    plt.xscale("log")
    plt.ylabel("minDCF")
    plt.title("Logistic Regression RAW - Different Working Points")

    plt.plot(l_values, results_raw_Pi_T_05, label="minDCF(Pi_T = 0.5)")
    plt.plot(l_values, results_raw_Pi_T_01, label="minDCF(Pi_T = 0.1)")
    plt.plot(l_values, results_raw_Pi_T_09, label="minDCF(Pi_T = 0.9)")
    
    plt.xlim(l_values[0], l_values[-1])
    plt.legend()
    plt.savefig("Training/Logistic_Regression_Plot/Logistic Regression RAW - Different Working Points.pdf")
    plt.close()
